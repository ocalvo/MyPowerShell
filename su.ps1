...
$RdpFilenam	= [System.Environment]::GetFolderPath([System.Environment+SpecialFolder]'MyDocuments') + "\Default.rdp"
...
try {
    ...
    if ( $FilePath )
    {
        ...
        else # No args
        {
            ...
            {
                Start-Process -LoadUserProfile -Credential:$PsCred -FilePath:PowerShell.exe -ArgumentList:("-nologo","-executionpolicy","bypass",$ScriptUnbArgs)
            }
        }
    }
    else
    {
        if ( $ScriptUnbArgs )
        {
            Start-Process -LoadUserProfile -Credential:$PsCred -FilePath:PowerShell.exe -ArgumentList:$ScriptUnbArgs
        }
        else
        {
            Start-Process -LoadUserProfile -Credential:$PsCred -FilePath:PowerShell.exe -ArgumentList:("-nologo","-executionpolicy","bypass")
        }
    }
}
catch {
