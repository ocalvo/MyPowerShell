...
New-Module -Name "oh-my-posh-core" -ScriptBlock {
    ...
    $script:OMPExecutable = 'C:/Users/ocalvo/AppData/Local/Programs/oh-my-posh/bin/oh-my-posh.exe'
    ...
    if (('' -ne '') -and (Test-Path -LiteralPath '')) {
        $env:POSH_THEME = (Resolve-Path -Path '').ProviderPath
    }
    ...
    if (("true" -eq "true") -and ($ExecutionContext.SessionState.LanguageMode -ne "ConstrainedLanguage")) {
