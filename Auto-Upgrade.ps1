...
while ($true)
{
  ...
  $build = dir ($buildShare + "\" + $branchName) -Dir | select -last 10 | where { test-path ($_.FullName+"\"+$arch+"fre\media\enterprise_en-us_vl") } | select -last 1
  ...
  if (($nextBuild -gt $currentBuild) -or (($nextBuild -eq $currentBuild) -and ($nextQFE -gt $currentQFE)))
  {
    ...
    $setupExe = $nextBuildShare + "\"+$arch+"fre\media\enterprise_en-us_vl\setup.exe"
